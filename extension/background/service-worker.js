...
async function initialize() {
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (Object.prototype.hasOwnProperty.call(changes, 'docsfocusManualOverrides')) {
...
async function handleManualOverrideMessage(message, sender, sendResponse) {
  ...
  try {
    if (enabled === true) {
      ...
      if (!granted) {
        sendResponse({ ok: false, error: 'Permission denied for this domain.' });
        return;
      }
    }
    ...
    const stateResponse = tabId != null ? await requestStateFromTab(tabId) : null;
...
async function injectIntoTab(tabId, domain) {
  ...
  try {
    ...
    console.warn('[DocsFocus] Unable to inject content script for domain', domain, error);
  }
}
...
async function ensureHostPermission(domain) {
  ...
  try {
    ...
    console.warn('[DocsFocus] Permission request failed for domain', domain, error);
    return false;
  }
}
