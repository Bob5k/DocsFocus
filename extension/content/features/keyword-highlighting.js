...
const SKIP_PROSE_SELECTOR = '.docsfocus-collapse, .docsfocus-code-preview, nav, header, footer, aside, code, pre, samp, kbd, script, style';
...
export function createKeywordHighlightFeature({ document, helpers }) {
  ...
  function activate(settings) {
    keywords = normalizeKeywords(settings.keywords ?? helpers.DEFAULT_SETTINGS.keywords);
  ...
  function update(settings) {
    const nextKeywords = normalizeKeywords(settings.keywords ?? helpers.DEFAULT_SETTINGS.keywords);
  ...
  function highlightProse() {
    ...
    targets.forEach((node) => wrapTextNode(node));
  }
  ...
  function clearProseHighlights() {
    const highlights = Array.from(document.querySelectorAll(`mark.${PROSE_MARK_CLASS}`));
  ...
  function highlightCodeBlocks() {
    ...
    blocks.forEach((block) => applyCodeOverlay(block));
  }
