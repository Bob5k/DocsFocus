...
const IGNORED_PARENTS =
	'nav, header, footer, aside, [role="navigation"], [data-docsfocus-ignore]';
...
export function createCollapsibleSectionsFeature({ document }) {
	...
	function enhanceHeading(heading, content, _sectionId) {
	...
	function restoreSection(entry) {
		const { heading, content, wrapper, onToggleClick, onHeadingKeyDown } =
			entry;
