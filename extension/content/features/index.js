export async function createFeatureManager(context) {
  ...
  const collapsibleSections = collapsibleSectionsModule.createCollapsibleSectionsFeature(context);
  const layoutTrim = layoutTrimModule.createLayoutTrimFeature(context);
  const navigationAids = navigationAidsModule.createNavigationAidsFeature(context);
