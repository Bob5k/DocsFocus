...
export function createNavigationAidsFeature({ document }) {
	...
	function handleIntersect(entries) {
		const indices = entries
			...
			.map((entry) =>
				headings.findIndex((item) => item.element === entry.target),
			)
	...
	function updateTrackerLabel() {
		...
		if (!headings.length) {
			...
			if (infoEl) {
				infoEl.textContent = settings.keyboardShortcuts
					? "Shift+J / Shift+K"
					: "";
			}
			return;
	...
	function handleKeydown(event) {
		...
		if (
			...
			(target.matches(FOCUSABLE_SELECTOR) || target.isContentEditable)
		) {
			return;
		}
	...
	function jumpToFirstCode() {
		const code = document.querySelector(
			".docsfocus-code-preview__code, main pre, pre, code",
		);
