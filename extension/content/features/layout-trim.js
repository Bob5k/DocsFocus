...
export function createLayoutTrimFeature({ document }) {
  ...
  function shouldTrimElement(element, viewportWidth, viewportHeight) {
    ...
    const computed = window.getComputedStyle(element);
    const descriptor = `${element.id} ${element.className} ${element.getAttribute('data-testid') || ''}`.toLowerCase();

    if (shouldTrimBanner(rect, computed, descriptor, viewportHeight)) {
      return true;
    }
    if (shouldTrimSidebar(rect, computed, descriptor, viewportWidth, viewportHeight)) {
      return true;
    }
  ...
  function shouldTrimBanner(rect, computed, descriptor, viewportHeight) {
    ...
    const isTopFixed = computed.position === 'fixed' || computed.position === 'sticky';
    const smallHeight = rect.height <= 180;
    const keywordMatch = KEYWORD_HINTS.some((keyword) => descriptor.includes(keyword));
    return isTopFixed && smallHeight && keywordMatch;
  }

  function shouldTrimSidebar(rect, computed, descriptor, viewportWidth, viewportHeight) {
    ...
    const sticky = computed.position === 'sticky' || computed.position === 'fixed';
    const keywordMatch = KEYWORD_HINTS.some((keyword) => descriptor.includes(keyword));
  ...
  function shouldTrimFloatingPanel(rect, computed, descriptor) {
    ...
    const nearEdge = rect.right >= (window.innerWidth || 0) - 60 || rect.left <= 60 || rect.bottom >= (window.innerHeight || 0) - 60;
    const keywordMatch = KEYWORD_HINTS.some((keyword) => descriptor.includes(keyword));
  ...
  function classifyElement(element) {
    ...
    if (descriptor.includes('banner') || descriptor.includes('announcement') || descriptor.includes('promo')) {
      return 'banner';
    }
    if (descriptor.includes('breadcrumb')) {
      return 'breadcrumbs';
    }
    if (descriptor.includes('toc') || descriptor.includes('table-of-contents')) {
      return 'toc';
    }
  ...
  function cleanupEntries() {
    Array.from(trimmedEntries.entries()).forEach(([element, entry]) => {
      if (!document.body.contains(element)) {
        trimmedEntries.delete(element);
        return;
      }
  ...
  function updateToggleButtonState() {
    ...
    toggleButton.textContent = previewVisible ? 'Hide trimmed UI' : 'Show trimmed UI';
  }
