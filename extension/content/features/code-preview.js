...
export function createCodePreviewFeature({ document, helpers }) {
  ...
  function generatePreviews() {
    const headings = getHeadings();
    headings.forEach((heading) => createOrUpdatePreview(heading));
  }

  function createOrUpdatePreview(heading) {
    if (!shouldShowPreview || !heading || heading.dataset.docsfocusPreview === 'true') {
      return;
    }
  ...
  function buildPreviewElement(heading, codeBlock, anchorId) {
    ...
    link.setAttribute('aria-label', `Jump to full example in section ${heading.textContent?.trim() || ''}`);
  ...
  function cloneCodeBlock(element) {
    const target = element.classList?.contains('highlight') && element.querySelector('pre')
      ? element
