(async function initDocsFocusContentScript() {
	try {
		...
		const state = {
			...
			autoDetectedInfo:
				describeDocsMatch?.(window.location.href, { document }) ?? null,
		...
		function ensureFeatureManager() {
			if (!featureManagerPromise) {
				featureManagerPromise = import(
					chrome.runtime.getURL("content/features/index.js")
				)
					.then(({ createFeatureManager }) =>
						createFeatureManager({ document, helpers }),
					)
					.catch((error) => {
		...
		function evaluateEligibility(overrides) {
			state.autoDetectedInfo = describeDocsMatch
				...
				: {
						matched: matchesDocsPattern(window.location.href, { document }),
						label: null,
					};
		...
		async function bootstrapState() {
			...
			const domainMap =
				domainSettingsMap && typeof domainSettingsMap === "object"
					? domainSettingsMap
					: {};
			const domainOverride = state.domain
				? (domainMap[state.domain] ?? null)
				: null;
			...
			state.settings = domainOverride
				? mergeSettings(normalizedGlobal, domainOverride)
				: normalizedGlobal;
		...
		function handleStorageChanges(changes, _areaName) {
			if (!changes) {
				return;
			}
			...
			maybeUpdate(STORAGE_KEYS.SETTINGS, (value) => {
				...
				const domainOverride = state.domain
					? (state.domainSettingsMap?.[state.domain] ?? null)
					: null;
				state.settings = domainOverride
					? mergeSettings(state.globalSettings, domainOverride)
					: state.globalSettings;
				state.domainPreset =
					domainOverride?.preset ?? state.globalSettings.preset;
				applyActivation();
			});
			...
			maybeUpdate(STORAGE_KEYS.DOMAIN_SETTINGS, (value) => {
				...
				const domainOverride = state.domain
					? (normalizedMap[state.domain] ?? null)
					: null;
				state.domainSettings = domainOverride;
				state.settings = domainOverride
					? mergeSettings(state.globalSettings, domainOverride)
